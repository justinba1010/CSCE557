...
from globally import DIRICHLETSIEVE, SIEVELIMIT, EXTRASAFETY, FERMATTEST

# DEBUG
DEBUG = False

def powm(a, b, m): #pylint: disable=invalid-name
    """
    Implementation of square and multiply exponentiation
    Unused, python's built in pow is used instead
    """
    if b == 0:
        return 1
    if b == 1:
...
def fermat_primality_test(num):
    ...
    return all([pow(i, num - 1, num) == 1\
            and (not EXTRASAFETY or sieve_erasthothenes(num))\
            for i in FERMATTEST])
